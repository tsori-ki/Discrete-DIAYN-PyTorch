...
class SAC:
    def __init__(self, **config):
        ...
        self.policy_network = PolicyNetwork(state_dim=self.state_shape, n_actions=self.n_actions).to(self.device)
        self.q_value_network1 = QValueNetwork(state_dim=self.state_shape, n_actions=self.n_actions).to(self.device)
        self.q_value_network2 = QValueNetwork(state_dim=self.state_shape, n_actions=self.n_actions).to(self.device)
        self.q_value_target_network1 = QValueNetwork(state_dim=self.state_shape, n_actions=self.n_actions).to(
            self.device)
        self.q_value_target_network2 = QValueNetwork(state_dim=self.state_shape, n_actions=self.n_actions).to(
            self.device)
    ...
    def set_rng_states(self, torch_state, random_state):
        import torch, random
